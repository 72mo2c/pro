...
const ManagePurchaseInvoices = () => {
  ...
  const handleDeleteConfirm = () => {
    try {
      deletePurchaseInvoice(invoiceToDelete.id);
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-4">
        ...
        <div className="mt-3 text-sm text-gray-600 flex flex-wrap items-center gap-4">
          <span>عرض {filteredInvoices.length} من {purchaseInvoices.length} فاتورة</span>
          {(dateFilterFrom || dateFilterTo || searchQuery || paymentTypeFilter !== 'all') && (
      ...
      {showDeleteConfirm && invoiceToDelete && (
        <ConfirmationModal
          ...
          title="تأكيد الحذف"
          message={`هل أنت متأكد من حذف الفاتورة #${invoiceToDelete.id}؟\nسيتم إعادة الكميات إلى المخزون.`}
          confirmText="حذف"
          cancelText="إلغاء"
...
const PurchaseReturnModal = ({ invoice, products, suppliers, purchaseReturns, onSubmit, onClose, formatCurrency, showError }) => {
  ...
  useEffect(() => {
    ...
    const itemsWithReturnInfo = invoice.items.map(item => {
      ...
      // حساب الكمية المرتجعة الأساسية والفرعية منفصلة
      ...
      // الحصول على الكمية الأصلية الأساسية والفرعية
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {returnItems.map((item, index) => {
                      const product = products.find(p => p.id === parseInt(item.productId));
                      const returnAmount = (item.returnMainQuantity || 0) * (item.originalMainPrice || 0) + 
                                          (item.returnSubQuantity || 0) * (item.originalSubPrice || 0);
