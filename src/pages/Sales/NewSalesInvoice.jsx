...
import { FaSave, FaPrint, FaSearch, FaTrash, FaPercent, FaMoneyBillWave, FaInfoCircle, FaExclamationTriangle, FaUserPlus, FaTimes, FaList, FaBarcode } from 'react-icons/fa';
...
const NewSalesInvoice = () => {
  ...
  const selectCustomer = (customer) => {
    ...
    setFormData({ 
      ...
      agentType: customer.agentType || '', // تحديد المندوب تلقائياً من بيانات العميل
      customerBalance: customerBalance // إضافة رصيد العميل
    });
    setCustomerSearch(customer.name);
    setShowCustomerSuggestions(false);
    
    // إظهار رسالة برصيد العميل
    if (customerBalance !== 0) {
      const balanceText = customerBalance > 0 ? `رصيد العميل: ${customerBalance.toFixed(2)} ج.م (له)` : `رصيد العميل: ${Math.abs(customerBalance).toFixed(2)} ج.م (عليه)`;
                          ...
                          {customer.balance !== undefined && (
                            <span className={`text-xs px-2 py-1 rounded ${
                              ...
                              {customer.balance === 0 ? 'متزن' : 
                               ...
                               `عليه: ${Math.abs(customer.balance).toFixed(2)}`} ج.م
                            </span>
                          )}
                        </div>
                      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
            ...
            <div className="bg-yellow-50 p-2 rounded-lg border border-yellow-200">
              ...
              {(formData.discountPercentage > 0 || formData.discountFixed > 0) && (
                <div className="text-xs text-gray-600 text-center bg-white px-1 py-0.5 rounded border border-yellow-200">
                  <span className="font-semibold text-red-600">{calculateDiscountAmount().toFixed(2)}</span>
                </div>
              )}
            </div>

            {/* المجموع ورصيد العميل */}
            <div className="bg-blue-50 p-2 rounded-lg border border-blue-200">
              <div className="space-y-1">
                ...
                {/* عرض رصيد العميل */}
                {getSelectedCustomerBalance() !== null && getSelectedCustomerBalance() !== undefined && (
                  <div className="mt-1 pt-1 border-t border-blue-300">
                    <div className="flex justify-between items-center">
                      <span className="text-xs font-semibold text-gray-700">رصيد العميل:</span>
                      <span className={`text-xs font-bold ${
                        getSelectedCustomerBalance() > 0 ? 'text-blue-600' : 
                        getSelectedCustomerBalance() < 0 ? 'text-red-600' : 
                        'text-green-600'
                      }`}>
                        {getSelectedCustomerBalance() === 0 ? '0.00' : getSelectedCustomerBalance().toFixed(2)}
                        {getSelectedCustomerBalance() > 0 && ' ج.م (له)'}
                        {getSelectedCustomerBalance() < 0 && ' ج.م (عليه)'}
                        {getSelectedCustomerBalance() === 0 && ' ج.م (متزن)'}
                      </span>
                    </div>
                  </div>
                )}
