...
const Sidebar = ({ isOpen, closeSidebar }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // فحص whether target is a valid Node
      const isValidTarget = target && 
                           typeof target.nodeType === 'number' && 
                           target.nodeType === 1;
      
      if (isValidTarget) {
        const sidebarContains = sidebarRef.current?.contains(target);
        const contextMenuContains = contextMenuRef.current?.contains(target);
        
        if (!sidebarContains && !contextMenuContains) {
          setActiveMenu(null);
        }
      }
    };
  ...
  const handleMouseLeave = (e) => {
    if (!isOpen) {
      ...
      // فحص whether relatedTarget is a valid Node قبل استخدام contains()
      const isValidNode = relatedTarget && 
                          typeof relatedTarget.nodeType === 'number' && 
                          relatedTarget.nodeType === 1;
      
      if (isValidNode) {
        const sidebarContains = sidebarRef.current?.contains(relatedTarget);
        const contextMenuContains = contextMenuRef.current?.contains(relatedTarget);
        ...
        // إذا لم يكن relatedTarget صحيحاً، فافترض أن الماوس خرج من الشريط الجانبي
        hoverTimeoutRef.current = setTimeout(() => {
          setIsHovered(false);
          setActiveMenu(null);
        }, 300);
